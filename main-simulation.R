...
for(iter in 1:Iterations){
	n = 100
	sam = generate_samples(n=n,p=p,p0=0.5)
	...
	sppoissregvb(X,y,init,prior="Bernulli", 
		eps = 1e-5, maxiter = 50)},error=function(e){NULL}))[[3]]
	...
	sppoissregvb(X,y,init,prior="Laplace", 
		eps = 1e-5, maxiter = 50)},error=function(e){NULL}))[[3]]
	pst[[5]] = tryCatch({fit[[5]]$mu_beta/sqrt(diag(fit[[5]]$sigma_beta)) > qnorm(1-0.05/p)},error=function(e){NULL})
	pst[[5]][1] = 1
...
#------------------------------------
#------------------------------------
pdf(file = "box11.pdf",width=14,height=7)
dseb = stack(as.data.frame(t(seb[,1:(iter-1)])))
names(dseb)[2] <- "Method"
myplot <- ggplot(dseb,aes(x = Method, y = values, color = Method, fill = Method)) + 
...
dev.off()
#------------------------------------
pdf(file = "box12.pdf",width=14,height=7)
dseb = stack(as.data.frame(t(se[,1:(iter-1)])))
names(dseb)[2] <- "Method"
myplot <- ggplot(dseb,aes(x = Method, y = values, color = Method, fill = Method)) + 
geom_boxplot(outlier.colour="black", outlier.shape=16,
outlier.size=2, notch=FALSE) + 
ylim(0, 0.5) + xlab('') + ylab('Train relative error')
suppressWarnings(print(myplot))
dev.off()
#------------------------------------
pdf(file = "box13.pdf",width=14,height=7)
dseb = stack(as.data.frame(t(set[,1:(iter-1)])))
names(dseb)[2] <- "Method"
myplot <- ggplot(dseb,aes(x = Method, y = values, color = Method, fill = Method)) + 
geom_boxplot(outlier.colour="black", outlier.shape=16,
outlier.size=2, notch=FALSE) + 
ylim(0, 0.5) + xlab('') + ylab('Test relative error')
suppressWarnings(print(myplot))
dev.off()
#------------------------------------
pdf(file = "box14.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box15.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box16.pdf",width=14,height=7)
dseb = stack(as.data.frame(t(syst[,1:(iter-1)])))
names(dseb)[2] <- "Method"
myplot <- ggplot(dseb,aes(x = Method, y = values, color = Method, fill = Method)) + 
geom_boxplot(outlier.colour="black", outlier.shape=16,
outlier.size=2, notch=FALSE) + 
ylim(0, 1.1) + xlab('') + ylab('Computation time')
suppressWarnings(print(myplot))
dev.off()
#------------------------------------
parsims = list()
