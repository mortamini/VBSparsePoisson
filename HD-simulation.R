...
for(iter in 1:Iterations){
	n = 30
	sam = generate_samples(n=n,p=p,p0=0.1, 
		mu_beta = 0.7, sigma_beta = 0.5, mu_x = 0.1, sigma_x = 0.05, 
		rhox = 0.3)
	...
	p0 = sum(pst[[1]])/p
	pst[[4]] = 1 * (abs(fit[[4]]$mu_beta)>sqrt(-2*c/(1+c)*(0.5*log(c)+log(p0/(1-p0)))*fit[[4]]$sigmapars$beta_sigma2/(fit[[4]]$sigmapars$alpha_sigma2 - 1)))
...
boxplot(t(seb),outline=F)
boxplot(t(se),outline=F)
boxplot(t(set),outline=F)
boxplot(t(FPR),outline=F)
boxplot(t(FNR),outline=F)
boxplot(t(syst),outline=F)
#------------------------------------
#------------------------------------
#-----------trimmed means------------
#------------------------------------
...
mod=jags.model(textConnection(mod_string),data=data,inits=inits,n.chain=1)
library(coda)
update(mod,1000)
...
#------------------------------------
#------------------------------------
pdf(file = "box11h.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box12h.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box13h.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box14h.pdf",width=14,height=7)
dseb = stack(as.data.frame(t(FPR[,1:(iter-1)])))
names(dseb)[2] <- "Method"
myplot <- ggplot(dseb,aes(x = Method, y = values, color = Method)) + 
geom_boxplot(fatten = 2, notch=FALSE,outlier.shape = NA) +
scale_y_continuous(limits = quantile(dseb$values, c(0.1, 0.9), na.rm = T)) +
ylim(0, 0.3) + xlab('') + ylab('False positive rate')
suppressWarnings(print(myplot))
dev.off()
#------------------------------------
pdf(file = "box15h.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box16h.pdf",width=14,height=7)
dseb = stack(as.data.frame(t(syst[,1:(iter-1)])))
names(dseb)[2] <- "Method"
myplot <- ggplot(dseb,aes(x = Method, y = values, color = Method)) + 
geom_boxplot(fatten = 2, notch=FALSE,outlier.shape = NA) +
scale_y_continuous(limits = quantile(dseb$values, c(0.1, 0.9), na.rm = T)) +
ylim(0, 10) + xlab('') + ylab('Computation time')
suppressWarnings(print(myplot))
dev.off()
#------------------------------------
pdf(file = "box17h.pdf",width=14,height=7)
...
for(h in 3:5){
	...
	plot(density(parsims[[h-2]][,j]),main = paste0(mcmcnames[h-2],"-prior posterior-density estimate for ",colnames(parsims[[h-2]])[j],"=",round(realbeta[j],3)),
	xlim = xlims,
	...
	qfunc = function(x) dnorm(x,fit[[h]]$mu_beta[j],sqrt(fit[[h]]$sigma_beta[j,j]))
...
xlims = c(0,1)
for(h in 3:4){
	ylims = c(0,max(density(parsims[[h-2]][,j])$y,max(limsy[2,])))
