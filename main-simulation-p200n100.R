...
for(iter in 1:Iterations){
	...
	mu_x = 0.1, sigma_x = 0.05,rhox = 0.3)
	...
	yhattest[[4]] = tryCatch({predict(fit[[4]],Xtest,method ="CS")},error=function(e){trunc(exp(Xtest %*% betahat[[4]]))})
	#------------------------------------
	cat("Laplace-VB \n")
	...
	yhattest[[5]] = tryCatch({predict(fit[[5]],Xtest,method ="Laplace")},error=function(e){trunc(exp(Xtest %*% betahat[[5]]))})
	#------------------------------------
	#------------------------------------
	#------------------------------------
	#
...
#------------------------------------
#------------------------------------
colMeans(CM <- Reduce("+",coverage)/Iterations)
colMeans(round(sqrt(Reduce("+",Map(function(x){(x-CM)^2},
...
#------------------------------------
#------------------------------------
pdf(file = "box41r.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box42r.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box43r.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box44r.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box45r.pdf",width=14,height=7)
...
dev.off()
#------------------------------------
pdf(file = "box46r.pdf",width=14,height=7)
...
colnames(cover) <- c("Bernoulli-VB","CS-VB",
"LAPLACE-VB")
pdf(file = "box47r.pdf",width=14,height=7)
